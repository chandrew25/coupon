        ...
        if($operation=='cj'){
        	include IA_ROOT . "/addons/tiger_newhu/inc/sdk/tbk/jd.php"; 
					...
					for ($x=1; $x<=$_GPC['num']; $x++) {
						...
						$arr['unionType']="1";
						$arr['type']="4";
						$arr['spaceNameList']=array($tgwname);
						$arr=array(
							'positionReq'=>$arr
						);			
						$goodslist=$this->cjget("jd.union.open.position.create",$arr);
						$arr=json_decode($goodslist,true);
						$arr=json_decode($goodslist,true);
						$tgw=json_decode($arr['jd_union_open_position_create_response']['result'],true);
