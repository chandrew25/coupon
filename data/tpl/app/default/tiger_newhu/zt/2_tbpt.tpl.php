...
<head>
    ...
    <title>超级拼团好货</title>
    <meta name="description" content="超级拼团好货" />
    <meta name="keywords" content="超级拼团好货" />
...
<body style="margin:0 auto;">
    ...
    <div class="slot-ban" id="top">
        <a href=""><img src="<?php  echo $_W['siteroot'];?>addons/tiger_newhu/template/mobile/qtz/images/pt.jpg" alt=""></a>
    </div>

    <div class="slot-tabs">
        <div class="header-nav" style="padding-right:0">
            <ul class="header-nav-list">
                <li class="item active js-nav-item" style="width: 50%;"><a href="javascript:;"   onclick="classtype(4071);"><span>聚划算拼团</span></a></li>
                <li class="item js-nav-item" style="width: 50%;"><a href="javascript:;"  onclick="classtype(6640);"><span>更多拼团</span></a></li>
							...
							layui.use('flow', function(){
								...
								flow.load({
									...
									,done: function(page, next){
										...
										$.get("<?php  echo $this->createMobileUrl('tbpt',array('tj'=>$tj,'key'=>$key,'tm'=>$tm,'typeid'=>$typeid,'pid'=>$pid,'zn'=>$zn,'lx'=>$lx))?>&page="+page+"&MaterialId="+MaterialId, function(res){
										// alert(res); 
										res=JSON.parse(res);
...
tmpl+='<li>';
		tmpl+='<div class="con">';
			tmpl+='<a href="<?php  echo $this->createMobileUrl("view")?>'+'&id='+item.id+'&dluid=<?php  echo $dluid;?>&lm='+item.lm+'&itemid='+item.itemid+'&org_price='+item.orig_price+'&price='+item.jdd_price+'&coupons_price='+item.couponmoney+'&goods_sale='+item.itemsale+'&title='+item.itemtitle+'&pic_url='+encodeURIComponent(item.itempic)+'&gopturl='+item.click_url+'">';
				tmpl+='<div class="pic">';
...
//					tmpl+='<div class="tips">奖152</div>';
				...
				tmpl+='<div class="txt">';
					...
					tmpl+='<div class="s1"><em>'+item.jdd_num+'人团</em></div>';
					tmpl+='<div class="s2">';
						tmpl+='<em>单买价'+item.orig_price+'</em>';
						tmpl+='<span>仅剩：'+item.stock+'件</span>';
					tmpl+='</div>';
					tmpl+='<div class="line" style="border:0;font-size:12px;height:14px">';
						tmpl+=''+item.oetime+'结束';
					tmpl+='</div>';
				tmpl+='</div>';
				tmpl+='<div class="quan">';
					tmpl+='<div class="q-con">';
						tmpl+='<div class="s1"><sub>￥</sub>'+item.jdd_price+'</div>';
						tmpl+='<div class="s2">拼团价</div>';
						tmpl+='<div class="s3">立即拼团</div>';
					tmpl+='</div>';
				tmpl+='</div>';
			tmpl+='</a>';
		tmpl+='</div>';
