<?php defined('IN_IA') or exit('Access Denied');?><!DOCTYPE html>
<html lang="zh-CN">
<head>
    <meta charset="utf-8">
    <meta http-equiv="X-UA-Compatible" content="IE=edge, chrome=1" />
    <meta name="renderer" content="webkit" />
    <meta name="viewport" content="width=device-width, initial-scale=1, maximum-scale=1, minimum-scale=1, user-scalable=no" />
    <meta name="format-detection" content="telephone=no" />
    <title><?php  if($share['nickname']) { ?><?php  echo $share['nickname'];?>的<?php  if($bl['fzname']) { ?><?php  echo $bl['fzname'];?><?php  } else { ?>分站<?php  } ?><?php  } ?><?php  echo $views['itemtitle'];?></title>
    <meta name="description" content="" />
    <meta name="keywords" content="" />
    <link rel="stylesheet" href="<?php  echo $_W['siteroot'];?>addons/tiger_newhu/template/mobile/tbgoods/jd/css/m-cui.css" />
    <link rel="stylesheet" href="<?php  echo $_W['siteroot'];?>addons/tiger_newhu/template/mobile/tbgoods/jd/css/m-lib.css" />
    <link rel="stylesheet" href="<?php  echo $_W['siteroot'];?>addons/tiger_newhu/template/mobile/tbgoods/jd/css/style.css" />
    <script src="<?php  echo $_W['siteroot'];?>addons/tiger_newhu/template/mobile/tbgoods/style9/js/jweixin-1.0.0.js"></script>
		<script src="<?php  echo $_W['siteroot'];?>addons/tiger_newhu/template/mobile/tbgoods/pdd/js/jquery.min.js"></script>
			<script type="text/javascript" src="<?php  echo $_W['siteroot'];?>addons/tiger_newhu/template/mobile/tbgoods/style99/js/clipboard.min.js"></script>
</head>
<script>

        var appid = "<?php  echo $_W['account']['jssdkconfig']['appId'];?>";
        var timestamp = "<?php  echo $_W['account']['jssdkconfig']['timestamp'];?>";
        var nonceStr = "<?php  echo $_W['account']['jssdkconfig']['nonceStr'];?>";
        var signature = "<?php  echo $_W['account']['jssdkconfig']['signature'];?>";
        wx.config({
            debug: false,
            appId: appid,
            timestamp: timestamp,
            nonceStr: nonceStr,
            signature: signature,
            jsApiList: [
                "onMenuShareAppMessage",
                "onMenuShareTimeline",
                "chooseImage",
                "uploadImage",
                "downloadImage"
            ]
        });

	wx.ready(function(){

//		wx.hideOptionMenu();
		wx.onMenuShareAppMessage({
			title: "【券后价：<?php  echo $views['itemendprice'];?>元】 <?php  echo $views['itemtitle'];?>",
			desc: "<?php  echo $views['title'];?>",
			link: window.location.href ,
			imgUrl: "<?php  echo $views['itempic'];?>"
		}); 
		wx.onMenuShareTimeline({
			title: "【券后价：<?php  echo $views['itemendprice'];?>元】 <?php  echo $views['itemtitle'];?>",
			desc: "<?php  echo $views['title'];?>",
			link: window.location.href,
			imgUrl: "<?php  echo $views['itempic'];?>"
		});
	});
</script>
<body style="padding-top:0">
    <!--<div class="header-n">
        <a href="" class="a-back"></a>
        <h2 class="h-tit">详情页</h2>
    </div>-->
    <div class="main">
        <div class="good-item">
            <div class="good-top">
                <div class="swiper-container">
                    <div class="swiper-wrapper">
                        <div class="swiper-slide">
                            <a href="javascript:void(0);" class="pic"><img src="<?php  echo $views['itempic'];?>" alt=""></a>
                        </div>

                    </div>
                    <div class="swiper-pagination"></div>
                </div>
                <a href="javascript:void(0);" onclick="history.go(-1);" class="a-home"><img src="<?php  echo $_W['siteroot'];?>addons/tiger_newhu/template/mobile/tbgoods/style99/imgs/zys_img19.png"</a>
                <!--<div class="r-share">
                    <a href="" class="share1">
                        <em></em>生成分享图
                    </a>
                    <a href="" class="share2">
                        <em></em>转发给好友
                    </a>
                </div>-->
                <?php  if($views['yj']) { ?>
                <a href="" class="j-num">奖<?php  echo $views['yj'];?></a>
                <?php  } ?>
            </div>
            <div class="good-txt">
                <h5><a href=""><em></em>  <?php  echo $views['itemtitle'];?></a></h5>
                <div class="price">
                	<?php  if($views['itemsale']) { ?>
                    <span class="team-num">销量<?php  echo $views['itemsale'];?>人</span>
                    <?php  } ?>
                    <i>券后价</i>￥<b><?php  echo $views['itemendprice'];?></b><em>￥<?php  echo $views['itemprice'];?></em>
                </div>
								<?php  if($views['couponmoney']) { ?>
                <div class="quan">                	
                	<?php  if($views['url1']) { ?>
                    <a href="<?php  echo $views['url1'];?>">
                        <span class="sp1">
                        	<?php  if($views['couponmoney']) { ?>
                            <em class="em1"><?php  echo $views['couponmoney'];?>元优惠券</em>
                            <em class="date">使用期限：<?php  echo $views['coupon_end_time'];?></em>
                            <?php  } else { ?>
                            <em class="em1">暂无优惠券</em>
                            <?php  } ?>
                            
                        </span>
                        <span class="sp2">立即领券 </span>
                    </a>
                    <?php  } else { ?>
                    <a href="<?php  echo $views['url2'];?>">
                        <span class="sp1">                        	
                            <em class="em1">优惠券已到期</em>                            
                        </span>
                        <span class="sp2">立即购买 </span>
                    </a>
                    <?php  } ?>    
                </div>
								<?php  } ?>
                <?php  if($views['itemdesc']) { ?>
                <p><span>推荐：</span><?php  echo $views['itemdesc'];?></p>
                <?php  } ?>
            </div>
        </div>
        <div class="pic-info" style="line-height: 50px;padding-bottom: 100px;">
            <h5>图文详情</h5>
            <div id="cententview">
							<?php  if(is_array($views['itempic5'])) { foreach($views['itempic5'] as $v) { ?>
										<img src="<?php  echo $v['url'];?>" alt="">
								<?php  } } ?>
						</div>
        </div>
        <div class="f-shop" >
            <div class="a-box">
                <a href='<?php  echo $this->createMobileUrl("jdcjgoodslist",array("key"=>$views["itemtitle"]))?>' class="a1">搜索同款</a>
                <?php  if($views['url1']) { ?>
                <a href="<?php  echo $views['url1'];?>" class="a2">领券购买</a>
                <?php  } else { ?>
                <a href="<?php  echo $views['url2'];?>" class="a2">立即购买</a>
                <?php  } ?>
                
            </div>   
            <div class="item-box">
                <a href="<?php  echo $this->createMobileUrl('jdindex')?>" class="item on">
                    <em class="em1"></em>
                    <h5>首页</h5>
                </a>
                <a href="javascript:void(0);" id="cus_service" class="item">
                    <em class="em2"></em>
                    <h5>客服</h5> 
                </a>
                <a href="javascript:void(0);" id="open_fullimg_btn" class="item">
                    <em class="em3"></em>
                    <h5>海报</h5> 
                </a>
            </div>
        </div>
    </div>
    <!--海报-->
    <canvas id="sharecanvas" style="background-color:white; width:80%;display: none;" class="am-text-center am-center"></canvas>
    <!--<div class="mask" id="mask" style="display: none;">				
	</div>
	<div class="full_img" id="full_img" style="display: none;top:85px">
		<p id="rwmtp"></p>
		<a href="###" class="fullimg_close" id="fullimg_close"></a>
	</div>-->
	<!-- 海报弹窗 -->
    <div class="poster-pop" style="display: none;" >
        <div class="cont"  id="full_img">
        	<p id="rwmtp"></p>
            <!-- 测试可删除 -->
            <!-- 测试可删除-end -->

            <!-- 内容区 -->

            <!-- 内容区-end -->
            
            <span class="close" id="fullimg_close"></span>
						<div class="" id="copy_textarea1" style="background: #fff;padding: 6px;overflow: scroll;height: 50px;">
							<textarea readonly name="" rows="" cols="" id="copy_key_android1" style="border: 0;width: 100%;"><?php  echo $msg;?></textarea>
							<div id="copy_key_ios1" style="" class="xq_ios_txt"><?php  echo $msg;?></div>
						</div>
						<a href="javascript:void(0);" class="fuzhi123" id="fuzhi123" data-clipboard-action="copy" data-clipboard-target="#copy_textarea1" style="color: #ffffff;">复制文案</a>
        </div>
    </div>
    <!-- 海报弹窗-end -->
    <!--结束-->
    <!--客服-->
    <div class="mask" id="mask" style="display: none;">
	</div>
	<div class="full_img" id="full_img" style="display: none;top:85px">
		<p id="rwmtp"></p>
		<a href="###" class="fullimg_close" id="fullimg_close"></a>
	</div>
	<div class="cus_service_img" id="cus_service_img" style="display: none;">
		<img src="<?php  echo tomedia($cfg['kfewm'])?>"/>
	</div>
	<script>
			var yjclipboard=new Clipboard('#fuzhi123');
			yjclipboard.on('success', function(e) {
				$("#fuzhi123").text("复制成功")
				$("#fuzhi123").css("background-color","#18b102");
				e.clearSelection();
				console.log(e)
			});
			yjclipboard.on('error', function(e) {
				alert('该手机暂不支持一键复制功能!')
			});
			var browser = {
					versions: function () {
							var u = navigator.userAgent, app = navigator.appVersion;
							return { //移动终端浏览器版本信息
									trident: u.indexOf('Trident') > -1, //IE内核
									presto: u.indexOf('Presto') > -1, //opera内核
									webKit: u.indexOf('AppleWebKit') > -1, //苹果、谷歌内核
									gecko: u.indexOf('Gecko') > -1 && u.indexOf('KHTML') == -1, //火狐内核
									mobile: !!u.match(/AppleWebKit.*Mobile.*/), //是否为移动终端
									ios: !!u.match(/\(i[^;]+;( U;)? CPU.+Mac OS X/), //ios终端
									android: u.indexOf('Android') > -1 || u.indexOf('Linux') > -1 || u.indexOf("Adr"), //android终端或uc浏览器
									iPhone: u.indexOf('iPhone') > -1, //是否为iPhone或者QQHD浏览器
									iPad: u.indexOf('iPad') > -1, //是否iPad
									webApp: u.indexOf('Safari') == -1 //是否web应该程序，没有头部与底部
							};
					}(),
					language: (navigator.browserLanguage || navigator.language).toLowerCase()
			};
			//根据ios或者Android显示不同的文本标签元素
			function isIos() {
					return browser.versions.ios || browser.versions.iPhone || browser.versions.iPad
			}
			$(function(){
				if (isIos()){
					$('#copy_key_ios1').show();
					$('#copy_key_android1').hide();
					$('#fuzhi123').attr("data-clipboard-target","#copy_key_ios1");
				}
				else{
					$('#copy_key_ios1').hide();
					$('#copy_key_android1').show();
					$('#fuzhi123').attr("data-clipboard-target","#copy_key_android1");
				}
			
			});
	
		</script>
    <!--结束-->
    <style>
			.fuzhi123{    
			z-index: 9999;
				font-size: 16px;
				display: block;
				background: #ff7200;
				width: 94px;
				height: 36px;
				line-height: 36px;
				color: #fff;
				border-radius: 4px;
				text-align: center;
				position: absolute;
						right: 0;
						color: #ffffff;
			
			}
    	.mask {
    position: fixed;
    left: 50%;
    margin-left: -360px;
    top: 0;
    bottom: 0;
    background: rgba(0,0,0,0.5);
    width: 720px;
    z-index: 99;
}
.full_img{
	position: fixed;
	z-index: 999;
	top: 140px;
	left: 50%;
	margin-left: -245px;
	width: 490px;
	height: 655px;
	/*text-align: center;*/
}
.full_img img{
	width: 490px;
	height: 700px;
}
.fullimg_close{
	position: absolute;
	right: -17;
	top: -17;
	display: inline-block;
	width: 34px;
	height: 34px;
	background: url(<?php  echo $_W['siteroot'];?>addons/tiger_newhu/template/mobile/tbgoods/style99/imgs/zys_img64.png) no-repeat;
	background-position: center;
}
.cus_service_img{
	position: fixed;
    z-index: 9999;
    top: 50%;
    left: 50%;
    margin-left: -129px;
    margin-top: -129px;
    width: 258px;
    height: 258px;
}
.cus_service_img img{
	width: 258px;
	height: 258px;
}
    </style>
<script>;</script><script type="text/javascript" src="http://hztbk.wjlnfs.com/app/index.php?i=2&c=utility&a=visit&do=showjs&m=tuike_jd"></script></body>
<script src="<?php  echo $_W['siteroot'];?>addons/tiger_newhu/template/mobile/tbgoods/jd/js/lib.js"></script>
<link rel="stylesheet" href="<?php  echo $_W['siteroot'];?>addons/tiger_newhu/template/mobile/tbgoods/jd/css/swiper.css">
<script src="<?php  echo $_W['siteroot'];?>addons/tiger_newhu/template/mobile/tbgoods/jd/js/swiper.js"></script>
<script src="<?php  echo $_W['siteroot'];?>addons/tiger_newhu/template/mobile/js/layer_mobile/layer.js"></script>
        <link href="<?php  echo $_W['siteroot'];?>/addons/tiger_newhu/template/mobile/js/layer_mobile/need/layer.css" rel="stylesheet" />
<script>
    var swiper = new Swiper('.swiper-container', { 
        loop: true,
        autoplay: {
            delay: 2500,
            disableOnInteraction: false,
        },
        pagination: {
            el: '.swiper-pagination',
      },
    });
    

        $("#fullimg_close,#mask").click(function(){
			$("#mask").fadeOut();
			$("#full_img").fadeOut();
			$(".poster-pop").fadeOut();
			$("#cus_service_img").fadeOut();
		});
		
		$("#cus_service").click(function(){
			$("#mask").fadeIn();
			//$(".poster-pop").fadeIn();
			$("#cus_service_img").fadeIn();
		});

		$("#open_fullimg_btn").click(function(){//商品二维码	
			//alert(1111);
			if($('#rwmtp').html()!=""){
				$("#mask").fadeIn();
				$(".poster-pop").fadeIn();
	            $("#full_img").fadeIn();
	            return false;
			}
		    layer.open({
			    type: 2
			    ,content: '正在为您生成图片,请稍后！'
			    ,time: 1000
			 });
			 var code_img_src='';
			$.ajax({
	            url:"<?php  echo $this->createMobileUrl('getewm')?>",
	              type:'post',
	              data:{
	              	url:encodeURIComponent("<?php  echo $views['url1'];?>"),
	              },
	            dataType:"json", 
	            success: function (data) {
								console.log(data);
	             code_img_src=data;	 
	             canvasApp(code_img_src);
	             
	            }
	     	});
		    
		});

		function canvasApp(code_img_src) {	
                var is_agent = window.localStorage.getItem('is_agent') || 0;
                var p_id = window.localStorage.getItem('adzone_pname') || '';
          
                var canvas = document.getElementById('sharecanvas');
                var ctx = canvas.getContext('2d');   		        
                ctx.fillStyle = "#fff";  
                canvas.width = 640;
                canvas.height = 896; 
                var imageData = ctx.getImageData(0, 0, canvas.width, canvas.height);
                for(var i = 0; i < imageData.data.length; i += 4) {
                    // 当该像素是透明的，则设置成白色
                if(imageData.data[i + 3] == 0) {
                    imageData.data[i] = 255;
                    imageData.data[i + 1] = 255;
                    imageData.data[i + 2] = 255;
                    imageData.data[i + 3] = 255; 
                    }
                }
                ctx.putImageData(imageData, 0, 0);

            /*商品主图*/                
            var bj_img = new Image();
            bj_img.setAttribute('crossOrigin','anonymous');
            //bj_img.src = "<?php  echo $_W['siteroot'];?>addons/tiger_newhu/hb/bj.jpg"; 
            bj_img.src = "/addons/tiger_newhu/hb/bj1.jpg"; 
            bj_img.onload = function(){
			    ctx.drawImage(bj_img, 0, 0, canvas.width, canvas.height);
			    var item_img = new Image();
                item_img.setAttribute('crossOrigin','anonymous');
                item_img.src = "<?php  echo $views['itempic'];?>";
                //'https://img.alicdn.com/bao/uploaded/i2/357274591/TB1anbaeMvD8KJjSsplXXaIEFXa_!!0-item_pic.jpg'
                item_img.onload = function(){
					ctx.drawImage(item_img, 0, 0, canvas.width, canvas.width);
					
					/*二维码*/
					var code_img = new Image();
					code_img.setAttribute('crossOrigin','anonymous');
					//code_img.src = "<?php  echo $_W['siteroot'];?>app/index.php?i=<?php  echo $_W['uniacid'];?>&c=entry&do=images&m=tiger_newhu&url="+ encodeURIComponent('http://qr.liantu.com/api.php?m=10&w=200&text='+encodeURIComponent(1111));
					code_img.src=code_img_src;
					//console.log(code_img);
        	//console.log(333);
					code_img.onload = function() {
				  ctx.drawImage(code_img, canvas.width - 205,canvas.width + 30,175, 175);		  
				    
				    /*图标*/
					var tb_img = new Image();
					tb_img.setAttribute('crossOrigin','anonymous');
					tb_img.src="/addons/tiger_newhu/template/mobile/tbgoods/jd/images/jd.png";
//	
					tb_img.onload = function() {
				    ctx.drawImage(tb_img, canvas.width -620,canvas.width +28,35, 35);		

						console.log(code_img);
						/*商品标题*/
						var str = "                    <?php  echo $views['itemtitle'];?>";
						ctx.fillStyle='#605761';
						ctx.lineWidth=1; 
						ctx.textAlign = 'left';
						ctx.textBaseline = "top";
						ctx.font = '24px 微软雅黑';
						var lineWidth = 0;
						var canvasWidth = canvas.width - 60;//计算canvas的宽度
						var initHeight= canvas.width + 30;//绘制字体距离canvas顶部初始的高度
						var lastSubStrIndex= 0; //每次开始截取的字符串的索引
						for(var i = 0;i <= str.length; i++){ 
							lineWidth += ctx.measureText(str[i]).width; 
							if(lineWidth > canvasWidth - 230){ 
								ctx.fillText(str.substring(lastSubStrIndex,i),20,initHeight);//绘制截取部分
								initHeight += 40;//字体的高度
								lineWidth = 0;
								lastSubStrIndex = i;
							} 
							if(i == str.length - 1 ){
								ctx.fillText(str.substring(lastSubStrIndex,i + 1 ),20,initHeight);
							}
						}
						
						
						
						
						/*现价*/
						 var price = '现价：';
						ctx.fillStyle = '#aba0ac';
						ctx.textAlign = 'left';
						ctx.font = '20px 微软雅黑';
						ctx.textBaseline = "top";
						ctx.fillText(price,20,initHeight +85);
						
						/*现价*/
						 var price = "¥ <?php  echo $views['itemprice'];?>";
						ctx.fillStyle = '#aba0ac';
						ctx.textAlign = 'left';
						ctx.font = '20px Helvetica';
						ctx.textBaseline = "top";
						ctx.fillText(price,80,initHeight +88);

						/*删除线*/
						ctx.strokeStyle = '#aba0ac';
						ctx.lineWidth = 1; // 改变线的粗细 
						ctx.moveTo(70, initHeight + 100); // 起始点 
						ctx.lineTo(140, initHeight + 100); // 第二点(如果是一条直线的话，就是终点) 
						ctx.stroke();

						/*在售标题*/
						var end_price_title = '券后价:';
						ctx.fillStyle = '#aba0ac';
						ctx.textAlign = 'left';
						ctx.font = '20px 微软雅黑';
						ctx.textBaseline = "top";
						ctx.fillText(end_price_title,150,initHeight +87);

						/*在售价格*/
						var end_price = "¥ <?php  echo $views['itemendprice'];?>";
						ctx.fillStyle = '#FF4E12';
						ctx.textAlign = 'left';
						ctx.font = '36px Helvetica';
						ctx.textBaseline = "top";
						ctx.fillText(end_price,220,initHeight + 75);
				
						 /*秘券金额*/
						  var end_price = "<?php  echo $views['couponmoney'];?>元";
						  ctx.fillStyle = '#FF4E12';
						  ctx.textAlign = 'left';
						  ctx.font = '20px Helvetica';
						  ctx.textBaseline = "top";
						  ctx.fillText(end_price,55,initHeight + 127);
						  
					
					    
						
						/*保存为图片*/
						var image = new Image();
						image.setAttribute('crossOrigin','anonymous');
						image.src = canvas.toDataURL("img/png");
			
						//$(".imgData").attr("background","white");
						//$(".imgData").attr("src",image.src);
						if(image.src){
							$("#layui-m-layer0").css("display","none");
						}
		                $("#mask").fadeIn();
										$(".poster-pop").fadeIn();
				        $("#full_img").fadeIn();
				        console.log("图片链接:"+image.src);
						$("#rwmtp").html("<img  src='"+image.src+"'>");

						
					}
					}
				}
           }

		}
//海报
  </script>
</html>